...
public class BrowserProvider extends ContentProvider {
    ...
    // sync com.android.browser.provider.Browser BOOKMARKS_URI
        ...
        switch (match) {
            case URI_MATCH_BOOKMARKS: {
                ...
                long rowID = db.insert(TABLE_NAMES[URI_MATCH_BOOKMARKS], "url",
                        initialValues);
                if (rowID > 0) {
                    uri = ContentUris.withAppendedId(BOOKMARKS_URI,
                            rowID);
                }
            ...
            case URI_MATCH_SEARCHES: {
                ...
                long rowID = db.insert(TABLE_NAMES[URI_MATCH_SEARCHES], "url",
                        initialValues);
                if (rowID > 0) {
                    uri = ContentUris.withAppendedId(SEARCHES_URI,
                            rowID);
                }
                break;
            }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        if (isBookmarkTable) {
            Cursor cursor = cr.query(BOOKMARKS_URI,
