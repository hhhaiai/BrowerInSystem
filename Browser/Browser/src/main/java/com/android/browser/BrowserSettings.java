...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    private Runnable mSetup = new Runnable() {
        ...
        public void run() {
            ...
            int x = RefMirror.on(ActivityManager.class).call("staticGetMemoryClass").get();
            if (x > 16) {
    ...
    private void syncSetting(WebSettings settings) {
        ...
        RefMirror.on(settings).call("setNavDump", enableNavDump()).get();
...
//        settings.setEnableSmoothTransition(true);
        RefMirror.on(settings).call("setEnableSmoothTransition", true).get();
        // disable content url access
        settings.setAllowContentAccess(false);

//        // HTML5 API flags
//        settings.setAppCacheEnabled(true);
        RefMirror.on(settings).call("setAppCacheEnabled", true).get();
        settings.setDatabaseEnabled(true);
        settings.setDomStorageEnabled(true);

        // HTML5 configuration parametersettings.
//        settings.setAppCacheMaxSize(getWebStorageSizeManager().getAppCacheMaxSize());
        RefMirror.on(settings).call("setAppCacheMaxSize", getWebStorageSizeManager().getAppCacheMaxSize()).get();
//        settings.setAppCachePath(getAppCachePath());
        RefMirror.on(settings).call("setAppCachePath", getAppCachePath()).get();
//        settings.setDatabasePath(mContext.getDir("databases", 0).getPath());
        RefMirror.on(settings).call("setDatabasePath", mContext.getDir("databases", 0).getPath()).get();
    ...
    public void clearHistory() {
        ...
        RefMirror.on(Browser.class).call("clearHistory", resolver).get();
        RefMirror.on(Browser.class).call("clearSearches", resolver).get();
