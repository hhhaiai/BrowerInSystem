...
//        ComponentName name = searchManager.getWebSearchActivity();
        ComponentName name = RefMirror.on(searchManager).call("getWebSearchActivity").get();
    ...
    public Cursor getSuggestions(Context context, String query) {
        SearchManager searchManager =
                (SearchManager) context.getSystemService(Context.SEARCH_SERVICE);
       return RefMirror.on(searchManager).call("getSuggestions",mSearchable, query).get();
