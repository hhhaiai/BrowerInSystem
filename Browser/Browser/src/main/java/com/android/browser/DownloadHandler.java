...
public class DownloadHandler {
    ...
    static void onDownloadStartNoStream(Activity activity,
        ...
        try {
            ...
            address = WebAddressHelper.getWebAddress(url);
            String p = encodePath(RefMirror.on(address).call("getPath").get());
            RefMirror.on(address).call("setPath", p).get();
        ...
        if (address==null){
            Log.e(LOGTAG, "2Exception trying to parse url:" + url);
            return;
        }

        String addressString = RefMirror.on(address).call("toString").get();
...
//        request.setDescription(webAddress.getHost());
        request.setDescription(RefMirror.on(address).call("getHost").get());
        // XXX: Have to use the old url since the cookies were stored using the
        // old percent-encoded url.
//        String cookies = CookieManager.getInstance().getCookie(url, privateBrowsing);
        String cookies = RefMirror.on(CookieManager.getInstance()).call("getCookie",url, privateBrowsing).get();
