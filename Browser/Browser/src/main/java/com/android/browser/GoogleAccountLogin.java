...
public class GoogleAccountLogin implements Runnable,
    ...
    public void run() {
        ...
        try {
            ...
            final Charset responseCharset = RefMirror.on("libcore.net.http.ResponseUtils").call("responseCharset", connection.getContentType()).get();
            byte[] responseBytes = RefMirror.on("libcore.io.Streams").call("readFully", connection.getInputStream()).get();
