...
//            imm.focusIn(mListView);
            RefMirror.on(imm).call("focusIn", mListView).get();
...
//                        url = address.toString();
                        Object address = WebAddressHelper.getWebAddress(unfilteredUrl);
                        if (address == null) {
                            throw new URISyntaxException("", "");
                        }
                        String host = RefMirror.on(address).call("getHost").get();
                        if (!TextUtils.isEmpty(host)) {
                            url = RefMirror.on(address).call("toString").get();
                        }
                    }
                }
