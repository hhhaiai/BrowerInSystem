...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
    ...
    static void deleteWidgetState(Context context, int widgetId) {
        File file = RefMirror.on(context).call("getSharedPrefsFile", String.format("widgetState-%d", widgetId)).get();
...
//                String.format("widgetState-%d", widgetId));
    ...
    static void removeOrphanedStates(Context context, int[] widgetIds) {
        File tmpFile= RefMirror.on(context).call("getSharedPrefsFile","null").get();
        File prefsDirectory = tmpFile.getParentFile();
    ...
    static class BookmarkFactory implements RemoteViewsService.RemoteViewsFactory {
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (isFolder) {
                ...
                RefMirror.on(views).call("setDrawableParameters",R.id.thumb, true, 0, -1, null, -1).get();
...
//                views.setDrawableParameters(R.id.thumb, true, 255, -1, null, -1);
                RefMirror.on(views).call("setDrawableParameters",R.id.thumb, true, 255, -1, null, -1).get();
